...
# ------------------------ Dataset Class for LSTM (Sequential Features) ------------------------
class BarkVoiceCommandDatasetLSTM(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # extract MFCC features for sequential processing
        with torch.no_grad():
            mfcc_features = self.mfcc_transform(waveform)
            mfcc_features = mfcc_features.squeeze(0).transpose(0, 1)  # (time_steps, n_mfcc)
        ...
        return mfcc_features, label_idx
...
def train_model(model, train_loader, val_loader, device, num_epochs=30):
    ...
    for epoch in range(num_epochs):
        ...
        if epoch_val_acc > best_val_acc:
            ...
            print(f"New best LSTM model saved with validation accuracy: {best_val_acc:.2f}%")
...
def main():
    ...
    print("Creating LSTM dataloaders...")
    train_dataset = BarkVoiceCommandDatasetLSTM(dataset_path, mode='train')
    val_dataset = BarkVoiceCommandDatasetLSTM(dataset_path, mode='val')
    ...
    # LSTM model with the number of classes from dataset
